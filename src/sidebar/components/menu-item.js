...
export default function MenuItem({
  ...
  if (href) {
    ...
    const buttonProps = {
      role: isSelected ? 'menuitemradio' : 'menuitem',
      'aria-checked': isRadioButtonType ? isSelected.toString() : false,
      'aria-expanded': hasSubmenuVisible
        ? isSubmenuVisible.toString()
        : undefined,
      'aria-haspopup': hasSubmenuVisible,
      //aria-label={`Show actions for ${label}`} // TODO, experiment with screen readers to
      // find the best affordance for this type of menu item
    };
    menuItem = (
      // FIXME, linter does not parse dynamic props (e.g. buttonProps), this
